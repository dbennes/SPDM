    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        if ($quantidadeAtual >= $quantidade) {
            ...
            if ($quantidadeAtual > 0) {
                // Adicionar no bd_pendencia apenas se a quantidade atual for maior que zero
                ...
                echo "<b> COD: </b> " . $codigo . " - <b> QTD: </b>". $quantidade ."<br> Quantidade insuficiente. Item com estoque zerado.<br><br>";
            }
        }
    } else {
        // Se um item não estiver disponível, marque a flag como falso
        ...
        if ($quantidadeAtual > 0) {
            ...
            echo "<b> COD: </b> " . $codigo . " - <b> QTD: </b>". $quantidade ."<br> Item não encontrado no estoque. Nenhuma ação necessária.<br><br>";
        }
    }
}
...
if ($allAvailable) {
    ...
    foreach ($cartTableData as $item) {
        ...
        // Verifica se o item está marcado como não disponível (bloco pendente)
        if (!$allAvailable) {
            // Verificar se o dado existe no bd_estoque
            $query = "SELECT * FROM bd_estoque WHERE CODIGO = '$codigo' FOR UPDATE";
            $result = mysqli_query($conn, $query);
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                // Adicionar no bd_pendencia apenas se não estiver já pendente e se a quantidade disponível for menor que a quantidade desejada
                $queryPendencia = "SELECT * FROM bd_pendencia WHERE CODIGO = '$codigo' AND PRIORIDADE = '$prioridade' AND REF = '$referencia' LIMIT 1";
                $resultPendencia = mysqli_query($conn, $queryPendencia);

                if (mysqli_num_rows($resultPendencia) == 0 && $quantidadeAtual < $quantidade) {
